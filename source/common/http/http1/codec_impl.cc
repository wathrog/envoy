...
int ConnectionImpl::onHeadersCompleteBase() {
  /*const absl::string_view header_type =
      processing_trailers_ ? Http1HeaderTypes::get().Trailers : Http1HeaderTypes::get().Headers;fixfix*/
  ASSERT(!processing_trailers_);
  ENVOY_CONN_LOG(trace, "onHeadersCompleteBas", connection_);
  completeLastHeader();
...
int ServerConnectionImpl::onHeadersComplete() {
  ...
  if (active_request_.has_value()) {
    ...
    if (connection_header_sanitization_ && headers->Connection()) {
